...
class ContestscraperSpider(scrapy.Spider):
    ...
    def parse_followed(self, response):
        try:
            img_id = self.santize(response.xpath('//div[@class="container"]//img//@src').get())
            self.image_ids.append(img_id)
            rating = response.xpath('//div[@class="container"]//p[contains(.,"Rating")]/span/text()').get()
    ...
    def parse(self, response):
        for item in response.xpath('//div[@class="row"][1]//div[@class="col-md-6"]//div[@class="gtco-icon"]'):
            try:
                item_id = self.santize(item.xpath('.//img//@src').get())
